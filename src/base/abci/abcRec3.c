...
//  int               nFilterThres; // filter threshold
    int               nAreaMargin; 	// area margin if limit area enabled
...
Vec_Wrd_t * Lms_GiaDelays( Gia_Man_t * p )
{
    ...
    Gia_ManForEachObj1( p, pObj, i )
    {
        ...
        else if ( Gia_ObjIsCo(pObj) )
		{
			...
			word delay = Lms_DelayDecrement( Vec_WrdEntry(vDelays, Gia_ObjFaninId0(pObj, i)), Gia_ManCiNum(p) );
			if (delay == 0 && Gia_ObjFaninId0(pObj, i) != 0)
			{
				printf("0 Delay! ObjID: %d, Fan0Id: %d\n", Gia_ObjId(p, pObj), Gia_ObjFaninId0(pObj, i));
			}
		}
...
***********************************************************************/
Lms_Man_t * Lms_ManStart( Gia_Man_t * pGia, int nVars, int nCuts, int fFuncOnly, int fVerbose, int nAreaMargin )
{
    Lms_Man_t * p;
...
//    Lms_ManPrintFreqStats( p );
    ...
    printf( "Subgraphs filtered by dump threshold        = %10d. (%6.2f %%)\n", p->nFilterDump,    !p->nTried? 0 : 100.0*p->nFilterDump/p->nAdded );
...
int Abc_NtkRecAddCut3( If_Man_t * pIfMan, If_Obj_t * pRoot, If_Cut_t * pCut )
{
	...
	// ++van
	ttSize = Abc_MinInt(nLeaves, p->nVars);
	spSize = Abc_TtSupportSize(If_CutTruthW(pCut), ttSize);
    if ( spSize < ttSize )
    {
//		printf("WARN - %s: cut size should be %d, however support size is %d\n", __func__, ttSize, spSize);
        p->nFilterSize++;
        return 1;
    }
	// --van
...
static inline int If_CutFindBestStructLimitArea( If_Man_t * pIfMan, If_Cut_t * pCut, If_Obj_t * pRoot, char * pCanonPerm, unsigned * puCanonPhase, int * pBestPo )
{
    Lms_Man_t * p = s_pMan3;
    int i, * pTruthId, iFirstPo, iFirstPoNext, iBestPo;
    int BestDelay = ABC_INFINITY, BestArea = ABC_INFINITY, Delay, Area, nOriginArea;
    int uSupport, nLeaves = If_CutLeaveNum( pCut );

    Vec_Ptr_t * vNodes = p->vNodes;
    If_CutTraverse( pIfMan, pRoot, pCut, vNodes );
	nOriginArea = Vec_PtrSize( vNodes );
...
p->timeCanon += Abc_Clock() - clk;
    ...
    for ( i = iFirstPo; i < iFirstPoNext; i++ )
    {
		...
		if ( (p->nAreaMargin >= 0) && (Area - nOriginArea > p->nAreaMargin) )	// limit area
			continue;
	...
	if (iBestPo == -1)
	{	// not found suitable structure with no area increasement
		assert(p->nAreaMargin >= 0);
        pCut->Cost = IF_COST_MAX;
        pCut->fUseless = 1;
        return ABC_INFINITY;
	}
	...
	// record freq
	Vec_IntAddToEntry(s_pMan3->vFreqs, iBestPo, 1);
...
int If_CutDelayRecCost3( If_Man_t * pIfMan, If_Cut_t * pCut, If_Obj_t * pObj )
{
    ...
    return ( s_pMan3->nAreaMargin < 0 ) ?
		If_CutFindBestStruct( pIfMan, pCut, pCanonPerm, &uCanonPhase, NULL ) :
		If_CutFindBestStructLimitArea( pIfMan, pCut, pObj, pCanonPerm, &uCanonPhase, NULL );
}
...
Hop_Obj_t * Abc_RecToHop3( Hop_Man_t * pMan, If_Man_t * pIfMan, If_Cut_t * pCut, If_Obj_t * pIfObj )
{
	...
	if ( s_pMan3->nAreaMargin < 0 )
		If_CutFindBestStruct( pIfMan, pCut, pCanonPerm, &uCanonPhase, &BestPo );
	else
		If_CutFindBestStructLimitArea( pIfMan, pCut, pIfObj, pCanonPerm, &uCanonPhase, &BestPo );
    assert( BestPo >= 0 );
...
//	If_CutFindBestStruct( pIfMan, pCut, pCanonPerm, &uCanonPhase, &BestPo );
	...
	{
		Abc_Print(ABC_ERROR, "Abc_RecToGia3 with area limit not implemented");
		assert(0);
//		If_CutFindBestStructLimitArea( pIfMan, pCut, pCanonPerm, &uCanonPhase, &BestPo );	// need root of If_Cut_t root
	}
    assert( BestPo >= 0 );
    pGiaPo = Gia_ManCo( pGia, BestPo );
...
Vec_Int_t * Lms_GiaFindNonRedundantCos( Lms_Man_t * p )
{
    ...
    Vec_IntForEachEntry( vUseful, EntryI, i )
    {
		...
		if (0 == D1) {
			Gia_Obj_t * pCo = Gia_ManCo(p->pGia, EntryI);
			int coId = Gia_ObjId(p->pGia, pCo);
			Abc_Print(ABC_ERROR, "zero delay in Entry: %d ObjId: %d Fan0Id: %d", EntryI, coId, Gia_ObjFaninId0(pCo, coId));
			exit(-1);
		}
        assert( D1 > 0 );
        Vec_IntForEachEntryStart( vUseful, EntryK, k, i+1 )
        {
			...
			if (0 == D2) {
				Gia_Obj_t * pCo = Gia_ManCo(p->pGia, EntryK);
				int coId = Gia_ObjId(p->pGia, pCo);
				Abc_Print(ABC_ERROR, "zero delay in Entry: %d ObjId: %d Fan0Id: %d", EntryK, coId, Gia_ObjFaninId0(pCo, coId));
				exit(-1);
			}
            assert( D2 > 0 );
    ...
    Vec_IntForEachEntry( vUseful, EntryI, i )
        if ( EntryI >= 0 )
		{
//			if ( p->nFilterThres && Vec_IntEntry(p->vFreqs, EntryI) < p->nFilterThres )
//			{	// filter low freq
//				p->nFilterDump++;
//				continue;
//			}
...
void Lms_GiaNormalize( Lms_Man_t * p )
{
    ...
    Gia_Obj_t * pObj;
    Vec_Int_t * vRemain;
    Vec_Int_t * vTruthIdsNew, * vFreqsNew;
...
//  vFreqsNew    = Vec_IntAlloc( Vec_IntSize(vRemain) );
...
//      Vec_IntPush( vFreqsNew, Vec_IntEntry(p->vFreqs, Entry) );
        Prev = Next;
    }
    // create a new GIA
...
Gia_Man_t * Lms_GetUseStatistic( Lms_Man_t * p )
{
	int i, j, Freq, Entry, sumCount, maxFreq;
	Vec_Int_t * vAccum;

	maxFreq = sumCount = 0;

	printf("Statistics: %d\n", Vec_IntSize(p->vFreqs));
    Vec_IntForEachEntry( p->vFreqs, Entry, i )
	{
		printf("\tAIG:\t%d\tFreq:\t%d\n", i, Entry);
		sumCount += Entry;
		if (Entry > maxFreq)
	...
	vAccum = Vec_IntStart( maxFreq + 1 );

    Vec_IntForEachEntry( p->vFreqs, Entry, i )
	{
		Vec_IntAddToEntry( vAccum, Entry, 1 * Entry );
	}

	printf("\n\n-----------------------------------------------------------\n\n");
	printf("Statistics 2: %d\n", maxFreq);

	Freq = 0;

	for (j = maxFreq; j >= 0; j--)
	{
		Entry = Vec_IntEntry( vAccum, j );
		if (Entry == 0)
			continue;
		Freq += Entry;
		printf("\tFreq >=%d\tPercent:\t%5.2f\n", j, (double)Freq / (double)sumCount );
	}
...
/// get a filtered GIA, need to release the mem after using the return value
/// @param p
/// @param nFilterThres threshold
/// @return 
Gia_Man_t * Lms_GetFilteredGia( Lms_Man_t * p, int nFilterThres )
{
	Gia_Man_t * pGiaCopy, * pGiaNew;
	Vec_Int_t * vRemain;
	int i, Freq, Entry;
	Gia_Obj_t * pObj;

	if (NULL == p || 0 != p->fLibConstr)
	{
		printf("LMS is now working for library constructing...\n");
		return 0;
	}

	assert( nFilterThres >= 0 );

	pGiaCopy = Gia_ManDup( p->pGia );
	if (0 == nFilterThres)
		return pGiaCopy;

	vRemain = Vec_IntAlloc( Vec_IntSize(p->vFreqs) );
	assert( Vec_IntSize(p->vFreqs) == Gia_ManCoNum(pGiaCopy) );
	Vec_IntForEachEntry( p->vFreqs, Freq, i )
	{
		if ( Freq >= nFilterThres )
			Vec_IntPush( vRemain, i );
	}

    Vec_IntForEachEntry( vRemain, Entry, i )
    {
        pObj = Gia_ManCo(pGiaCopy, Entry);
        assert( Gia_ObjIsAnd(Gia_ObjFanin0(pObj)) );
        Vec_IntWriteEntry( vRemain, i, Gia_ObjFaninLit0p(pGiaCopy, pObj) );
    }
    // create a new GIA
    Gia_ManForEachObj( pGiaCopy, pObj, i )
        assert( pObj->fMark0 == 0 );
    for ( i = 0; i < Gia_ManCoNum(pGiaCopy); i++ )
        Gia_ManPatchCoDriver( pGiaCopy, i, 0 );
    Vec_IntForEachEntry( vRemain, Entry, i )
        Gia_ManAppendCo( pGiaCopy, Entry );
    pGiaNew = Gia_ManCleanupOutputs( pGiaCopy, Gia_ManCoNum(pGiaCopy) - Vec_IntSize(vRemain) );
    Gia_ManStop( pGiaCopy );
    Vec_IntFree( vRemain );
	return pGiaNew;
}
...
Gia_Man_t * Abc_NtkRecGetFilteredGia( int nLimit )
{
	return Lms_GetFilteredGia( s_pMan3, nLimit );
}

void Abc_NtkRecGetFilteredStatistic()
{
	return Lms_GetUseStatistic( s_pMan3 );
}

void Abc_NtkRecPs3(int fPrintLib)
{
    Lms_ManPrint( s_pMan3 );
#ifdef PRINT_HASH_INFO
	Vec_MemPS( s_pMan3->vTtMem );
#endif
}
void Abc_NtkRecStart3( Gia_Man_t * p, int nVars, int nCuts, int fFuncOnly, int fVerbose, int nAreaMargin )
{
    assert( s_pMan3 == NULL );
    s_pMan3 = Lms_ManStart( p, nVars, nCuts, fFuncOnly, fVerbose, nAreaMargin );
}
